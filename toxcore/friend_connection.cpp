...
int friend_add_tcp_relay(Friend_Connections *fr_c, int friendcon_id, IPPort ip_port, const PublicKey &public_key)
{
    ...
    return fr_c->net_crypto->add_tcp_relay_peer(friend_con->crypt_connection_id, ip_port, public_key);
}
...
static void connect_to_saved_tcp_relays(Friend_Connections *fr_c, int friendcon_id, unsigned int number)
{
    ...
    for (i = 0; (i < FRIEND_MAX_STORED_TCP_RELAYS) && (number != 0); ++i) {
        ...
        if (friend_con->tcp_relays[index].ip_port.ip.family != Family::FAMILY_NULL) {
            if (fr_c->net_crypto->add_tcp_relay_peer(friend_con->crypt_connection_id, friend_con->tcp_relays[index].ip_port,
...
static unsigned int send_relays(Friend_Connections *fr_c, int friendcon_id)
{
    ...
    n = fr_c->net_crypto->copy_connected_tcp_relays(nodes, MAX_SHARED_RELAYS);
    ...
    if (fr_c->net_crypto->write_cryptpacket(friend_con->crypt_connection_id, data, length, 0) != -1) {
...
static void dht_ip_callback(void *object, int32_t number, IPPort ip_port)
{
    Friend_Connections *fr_c = (Friend_Connections *) object;
    Friend_Conn *friend_con = get_conn(fr_c, number);
...
static void change_dht_pk(Friend_Connections *fr_c, int friendcon_id, const PublicKey &dht_public_key)
{
    ...
    if (friend_con->dht_lock) {
        if (fr_c->dht->delfriend(friend_con->dht_temp_pk, friend_con->dht_lock) != 0) {
...
static int send_ping(const Friend_Connections *fr_c, int friendcon_id)
{
    ...
    int64_t ret = fr_c->net_crypto->write_cryptpacket(friend_con->crypt_connection_id, &ping, sizeof(ping), 0);
                               ...
                               uint16_t length)
{
    ...
    if (friend_con->status == FriendConnectionStatus::FRIENDCONN_STATUS_CONNECTED) {
        ...
        return fr_c->net_crypto->write_cryptpacket(friend_con->crypt_connection_id, packet, sizeof(packet), 0) != -1;
    } else {
