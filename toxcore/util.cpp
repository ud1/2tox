...
{
    memcpy(dest, src, crypto_box_BEFORENMBYTES);
...
{
    RingBuffer *buf = reinterpret_cast<RingBuffer *>(calloc(sizeof(RingBuffer), 1));

    if (!buf) return NULL;

    buf->size = size + 1; /* include empty elem */

    if (!(buf->data = reinterpret_cast<void **>(calloc(buf->size, sizeof(void *))))) {
        free(buf);
        return NULL;
    }
