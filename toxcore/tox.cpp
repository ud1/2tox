...
#include "../toxencryptsave/defines.hpp"

#define TOX_DEFINED
typedef Messenger Tox;
...
{
    struct Tox_Options *options = (Tox_Options *) calloc(sizeof(struct Tox_Options), 1);
...
Tox *tox_new(const struct Tox_Options *options, TOX_ERR_NEW *error)
{
    ...
    Messenger *m = new Messenger(&m_options);
    ...
    if (load_savedata_tox && m->messenger_load(options->savedata_data, options->savedata_length) == -1) {
        SET_ERROR_PARAMETER(error, TOX_ERR_NEW_LOAD_BAD_FORMAT);
...
{
    const Messenger *m = tox;
    return m->messenger_run_interval();
}
...
bool tox_self_set_status_message(Tox *tox, const uint8_t *status, size_t length, TOX_ERR_SET_INFO *error)
{
    ...
    if (m->m_set_statusmessage(status, length) == 0) {
        SET_ERROR_PARAMETER(error, TOX_ERR_SET_INFO_OK);
        return 1;
    } else {
        SET_ERROR_PARAMETER(error, TOX_ERR_SET_INFO_TOO_LONG);
...
{
    const Messenger *m = tox;
    return m->m_get_self_statusmessage_size();
}
...
void tox_self_get_status_message(const Tox *tox, uint8_t *status)
{
    if (status) {
        const Messenger *m = tox;
        m->m_copy_self_statusmessage(status);
    }
}
...
{
    Messenger *m = tox;
    m->m_set_userstatus(user_status);
}

int tox_self_get_status(const Tox *tox)
{
    const Messenger *m = tox;
    return m->m_get_self_userstatus();
}
                        ...
                        TOX_ERR_FRIEND_ADD *error)
{
    ...
    Messenger *m = tox;
    int32_t ret = m->m_addfriend(address, message, length);
...
uint32_t tox_friend_add_norequest(Tox *tox, const uint8_t *public_key, TOX_ERR_FRIEND_ADD *error)
{
    ...
    Messenger *m = tox;
    int32_t ret = m->m_addfriend_norequest(PublicKey(public_key));
...
{
    Messenger *m = tox;
    int ret = m->delete_friend(friend_number);
...
uint32_t tox_friend_by_public_key(const Tox *tox, const uint8_t *public_key, TOX_ERR_FRIEND_BY_PUBLIC_KEY *error)
{
    ...
    const Messenger *m = tox;
    int32_t ret = m->getfriend_id(PublicKey(public_key));
                               ...
                               TOX_ERR_FRIEND_GET_PUBLIC_KEY *error)
{
    ...
    const Friend *f = m->get_friend(friend_number);
    ...
    memcpy(public_key, f->real_pk.data.data(), f->real_pk.data.size());
...
{
    const Messenger *m = tox;
    return m->get_friend(friend_number) != nullptr;
}

uint64_t tox_friend_get_last_online(const Tox *tox, uint32_t friend_number, TOX_ERR_FRIEND_GET_LAST_ONLINE *error)
{
    const Messenger *m = tox;
    uint64_t timestamp = m->get_friend(friend_number)->m_get_last_online();
...
void tox_self_get_friend_list(const Tox *tox, uint32_t *list)
{
    if (list) {
        const Messenger *m = tox;
        //TODO: size parameter?
        m->copy_friendlist(list, tox_self_get_friend_list_size(tox));
    }
}
...
{
    const Messenger *m = tox;
    int ret = m->get_friend(friend_number)->m_get_name_size();
...
bool tox_friend_get_name(const Tox *tox, uint32_t friend_number, uint8_t *name, TOX_ERR_FRIEND_QUERY *error)
{
    ...
    const Messenger *m = tox;
    int ret = m->get_friend(friend_number)->getname(name);
...
{
    Messenger *m = tox;
    //m_callback_namechange(m, function, user_data); TODO
}

size_t tox_friend_get_status_message_size(const Tox *tox, uint32_t friend_number, TOX_ERR_FRIEND_QUERY *error)
{
    const Messenger *m = tox;
    int ret = m->get_friend(friend_number)->m_get_statusmessage_size();
                                   ...
                                   TOX_ERR_FRIEND_QUERY *error)
{
    ...
    const Messenger *m = tox;
    //TODO: size parameter?
    int ret = m->get_friend(friend_number)->m_copy_statusmessage(message, m->get_friend(friend_number)->m_get_statusmessage_size());
...
{
    Messenger *m = tox;
    //m_callback_statusmessage(m, function, user_data); TODO
}

int tox_friend_get_status(const Tox *tox, uint32_t friend_number, TOX_ERR_FRIEND_QUERY *error)
{
    const Messenger *m = tox;

    int ret = m->get_friend(friend_number)->m_get_userstatus();
...
{
    Messenger *m = tox;
    // m_callback_userstatus(m, function, user_data); TODO
}

int tox_friend_get_connection_status(const Tox *tox, uint32_t friend_number, TOX_ERR_FRIEND_QUERY *error)
{
    const Messenger *m = tox;

    int ret = m->get_friend(friend_number)->m_get_friend_connectionstatus();
...
{
    Messenger *m = tox;
    //m_callback_connectionstatus(m, function, user_data); TODO
}

bool tox_friend_get_typing(const Tox *tox, uint32_t friend_number, TOX_ERR_FRIEND_QUERY *error)
{
    const Messenger *m = tox;
    int ret = m->get_friend(friend_number)->m_get_istyping();
...
{
    Messenger *m = tox;
    //m_callback_typingchange(m, function, user_data); TODO
}
...
bool tox_self_set_typing(Tox *tox, uint32_t friend_number, bool is_typing, TOX_ERR_SET_TYPING *error)
{
    ...
    if (m->get_friend(friend_number)->m_set_usertyping(is_typing) == -1) {
                                 ...
                                 size_t length, TOX_ERR_FRIEND_SEND_MESSAGE *error)
{
    ...
    set_message_error(m->get_friend(friend_number)->m_send_message_generic(type, message, length, &message_id), error);
    return message_id;
}
...
{
    Messenger *m = tox;
    //m_callback_read_receipt(m, function, user_data); TODO
}
...
{
    Messenger *m = tox;
    //m_callback_friendmessage(m, function, user_data); TODO
}
...
{
    Messenger *m = tox;
    int ret = m->get_friend(friend_number)->file_control(file_number, control);
...
{
    Messenger *m = tox;
    int ret = m->get_friend(friend_number)->file_seek(file_number, position);
...
{
    Messenger *m = tox;
    //callback_file_control(m, function, user_data); TODO
}
                          ...
                          TOX_ERR_FILE_GET *error)
{
    ...
    const Messenger *m = tox;
    int ret = m->get_friend(friend_number)->file_get_id(file_number, file_id);
                       ...
                       const uint8_t *filename, size_t filename_length, TOX_ERR_FILE_SEND *error)
{
    ...
    Messenger *m = tox;
    long int file_num = m->get_friend(friend_number)->new_filesender(kind, file_size, file_id, filename, filename_length);
...
{
    Messenger *m = tox;
    int ret = m->get_friend(friend_number)->file_data(file_number, position, data, length);
...
{
    Messenger *m = tox;
    //callback_file_reqchunk(m, function, user_data); TODO
}
...
{
    Messenger *m = tox;
    //callback_file_data(m, function, user_data); TODO
}
                                  ...
                                  TOX_ERR_FRIEND_CUSTOM_PACKET *error)
{
    ...
    int ret = m->get_friend(friend_number)->send_custom_lossy_packet(data, length);
...
{
    Messenger *m = tox;
    //custom_lossy_packet_registerhandler(m, function, user_data); TODO
}
                                     ...
                                     TOX_ERR_FRIEND_CUSTOM_PACKET *error)
{
    ...
    int ret = m->get_friend(friend_number)->send_custom_lossless_packet(data, length);
...
{
    Messenger *m = tox;
    //custom_lossless_packet_registerhandler(m, function, user_data); TODO
}
