...
static unsigned int ping_node_from_getnodes_ok(DHT *dht, const uint8_t *public_key, IP_Port ip_port)
{
    ...
    for (i = 0; i < dht->num_friends; ++i) {
        ...
        DHT_Friend *friend_ = &dht->friends_list[i];
                  ...
                  void *data, int32_t number, uint16_t *lock_count)
{
    ...
    if (friend_num != -1) { /* Is friend_ already in DHT? */
        DHT_Friend *friend_ = &dht->friends_list[friend_num];
    ...
    DHT_Friend *temp;
    temp = reinterpret_cast<DHT_Friend *>(realloc(dht->friends_list, sizeof(DHT_Friend) * (dht->num_friends + 1)));

    if (temp == NULL)
        return -1;

    dht->friends_list = temp;
    DHT_Friend *friend_ = &dht->friends_list[dht->num_friends];
    ...
    friend_->num_to_bootstrap = get_close_nodes(dht, friend_->public_key, friend_->to_bootstrap, 0, 1, 0);
...
int DHT_delfriend(DHT *dht, const uint8_t *public_key, uint16_t lock_count)
{
    ...
    temp = reinterpret_cast<DHT_Friend *>(realloc(dht->friends_list, sizeof(DHT_Friend) * (dht->num_friends)));
...
static void do_DHT_friends(DHT *dht)
{
    ...
    for (i = 0; i < dht->num_friends; ++i) {
        ...
        for (j = 0; j < friend_->num_to_bootstrap; ++j) {
            getnodes(dht, friend_->to_bootstrap[j].ip_port, friend_->to_bootstrap[j].public_key, friend_->public_key, NULL);
        }

        friend_->num_to_bootstrap = 0;

        do_ping_and_sendnode_requests(dht, &friend_->lastgetnode, friend_->public_key, friend_->client_list, MAX_FRIEND_CLIENTS,
...
int route_tofriend(const DHT *dht, const uint8_t *friend_id, const uint8_t *packet, uint16_t length)
{
    ...
    DHT_Friend *friend_ = &dht->friends_list[num];
    Client_data *client;
