	...
	for (i = 0; i < num_friends; ++i)
	{
										...
										MAX_FRIEND_CLIENTS, public_key, ip_port))
		{
							 ...
							 public_key, ip_port, friends_list[i].public_key))
			{
				...
				if (public_key_cmp (public_key, friend_->public_key) == 0)
				{
...
int DHT::returnedip_ports (IP_Port ip_port, const uint8_t *public_key, const uint8_t *nodepublic_key)
{
	...
	if (id_equal (public_key, self_public_key))
	{
		...
		{
			if (id_equal (nodepublic_key, close_clientlist[i].public_key))
			{
				if (ip_port.ip.family == AF_INET)
				{
	...
	else
	{
		for (i = 0; i < num_friends; ++i)
		{
			if (id_equal (public_key, friends_list[i].public_key))
			{
				for (j = 0; j < MAX_FRIEND_CLIENTS; ++j)
				{
					if (id_equal (nodepublic_key, friends_list[i].client_list[j].public_key))
					{
						if (ip_port.ip.family == AF_INET)
						{
					 ...
					 const Node_format *sendback_node)
{
	...
	if (sendback_node != NULL)
	{
		...
		ping_id = ping_array_add (&dht_harden_ping_array, plain_message, sizeof (plain_message));
	}
	else
	{
		ping_id = ping_array_add (&dht_ping_array, plain_message, sizeof (receiver));
	}
									 ...
									 Node_format *sendback_node)
{
	...
	if (ping_array_check (data, sizeof (data), &dht_ping_array, ping_id) == sizeof (Node_format))
	{
		memset (sendback_node, 0, sizeof (Node_format));
	}
	else if (ping_array_check (data, sizeof (data), &dht_harden_ping_array, ping_id) == sizeof (data))
	{
				   ...
				   void *data, int32_t number, uint16_t *lock_count)
{
	...
	DHT_Friend *temp;
	temp = reinterpret_cast<DHT_Friend *> (realloc (friends_list, sizeof (DHT_Friend) * (num_friends + 1)));

	if (temp == NULL)
		return false;

	friends_list = temp;
	DHT_Friend *friend_ = &friends_list[num_friends];
	memset (friend_, 0, sizeof (DHT_Friend));
	memcpy (friend_->public_key, public_key, crypto_box_PUBLICKEYBYTES);
...
bool DHT::delfriend (const uint8_t *public_key, uint16_t lock_count)
{
	...
	if (num_friends != friend_num)
	{
		memcpy (&friends_list[friend_num],
				&friends_list[num_friends],
				sizeof (DHT_Friend));
	}
	...
	temp = reinterpret_cast<DHT_Friend *> (realloc (friends_list, sizeof (DHT_Friend) * (num_friends)));
...
int DHT::getfriendip (const uint8_t *public_key, IP_Port *ip_port) const
{
	...
	{
		/* Equal */
		if (id_equal (friends_list[i].public_key, public_key))
		{
			for (j = 0; j < MAX_FRIEND_CLIENTS; ++j)
			{
				Client_data *client = &friends_list[i].client_list[j];
...
int DHT::route_packet (const uint8_t *public_key, const uint8_t *packet, uint16_t length) const
{
	...
	for (i = 0; i < LCLIENT_LIST; ++i)
	{
		if (id_equal (public_key, close_clientlist[i].public_key))
		{
...
int DHT::friend_iplist (IP_Port *ip_portlist, uint16_t friend_num) const
{
	...
	for (i = 0; i < MAX_FRIEND_CLIENTS; ++i)
	{
		...
		if (id_equal (client->public_key, friend_->public_key))
...
int DHT::send_NATping (const uint8_t *public_key, uint64_t ping_id, uint8_t type)
{
	...
	int len = create_request (self_public_key, self_secret_key, packet, public_key, data,
...
void DHT::punch_holes (IP ip, uint16_t *port_list, uint16_t numports, uint16_t friend_num)
{
	...
	if (i == numports)   /* If all ports are the same, only try that one port. */
	{
		...
		send_ping_request (ping, pinging, friends_list[friend_num].public_key);
	}
	else
	{
...
int DHT::send_hardening_req (Node_format *sendto, uint8_t type, uint8_t *contents, uint16_t length)
{
	...
	int len = create_request (self_public_key, self_secret_key, packet, sendto->public_key, data,
...
IPPTsPng *DHT::get_closelist_IPPTsPng (const uint8_t *public_key, sa_family_t sa_family)
{
	...
	for (i = 0; i < LCLIENT_LIST; ++i)
	{
		if (public_key_cmp (close_clientlist[i].public_key, public_key) != 0)
			continue;
...
uint32_t DHT::have_nodes_closelist (Node_format *nodes, uint16_t num)
{
	...
	for (i = 0; i < num; ++i)
	{
		if (id_equal (nodes[i].public_key, self_public_key))
		{
			++counter;
			continue;
		}
...
void DHT::do_hardening ()
{
	...
	for (i = 0; i < LCLIENT_LIST * 2; ++i)
	{
		...
		uint8_t   *public_key = close_clientlist[i / 2].public_key;
...
DHT *new_DHT (Networking_Core *net)
{
	...
	DHT *dht = reinterpret_cast<DHT *> (calloc (1, sizeof (DHT)));
	...
	dht->cryptopacket_registerhandler (CRYPTO_PACKET_NAT_PING, &handle_NATping, dht);
	dht->cryptopacket_registerhandler (CRYPTO_PACKET_HARDENING, &handle_hardening, dht);
	...
	for (i = 0; i < DHT_FAKE_FRIEND_NUMBER; ++i)
	{
		...
		if (!dht->addfriend (random_key_bytes, 0, 0, 0, 0))
		{
			kill_DHT (dht);
			return NULL;
		}
...
void kill_DHT (DHT *dht)
{
	...
	dht->cryptopacket_registerhandler (CRYPTO_PACKET_NAT_PING, NULL, NULL);
	dht->cryptopacket_registerhandler (CRYPTO_PACKET_HARDENING, NULL, NULL);
...
uint32_t DHT::size () const
{
	...
	for (i = 0; i < DHT_FAKE_FRIEND_NUMBER && i < num_friends; ++i)
	{
...
void DHT::save (uint8_t *data)
{
	...
	for (num = 0, i = 0; i < LCLIENT_LIST; ++i)
	{
		...
		{
			memcpy (clients[num].public_key, close_clientlist[i].public_key, crypto_box_PUBLICKEYBYTES);
...
bool DHT::connect_after_load ()
{
	...
	{
		free (loaded_nodes_list);
		loaded_nodes_list = NULL;
		loaded_num_nodes = 0;
		return true;
	}
	...
	switch (type)
	{
		case DHT_STATE_TYPE_NODES:
...

			{
				...
				dht->loaded_nodes_list = reinterpret_cast<Node_format *> (calloc (MAX_SAVED_DHT_NODES, sizeof (Node_format)));
