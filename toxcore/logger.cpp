...
#include "logger.hpp"
#include "crypto_core.hpp" /* for random_int() */
...
uint64_t current_time_monotonic(void); // network.cpp
...
Logger *logger_new (const char *file_name, LOG_LEVEL level, const char *id)
{
    ...
    Logger *retu = reinterpret_cast<Logger *>(calloc(1, sizeof(Logger)));
    ...
    if (!(retu->tstr = reinterpret_cast<char *>(calloc(16, sizeof (char)))) ||
            !(retu->posstr = reinterpret_cast<char *>(calloc(300, sizeof (char)))) ||
            !(retu->msg = reinterpret_cast<char *>(calloc(4096, sizeof (char)))))
        goto FAILURE;

    if (id) {
        if (!(retu->id = reinterpret_cast<char *>(calloc(strlen(id) + 1, 1))))
            goto FAILURE;

        strcpy(retu->id, id);
    } else {
        if (!(retu->id = reinterpret_cast<char *>(malloc(8))))
            goto FAILURE;
